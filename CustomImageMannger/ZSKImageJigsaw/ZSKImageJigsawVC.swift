...
public class ZSKImageJigsawVC: UIViewController {
    ...
    public override func viewDidLoad() {
        super.viewDidLoad()
        view.backgroundColor = .zsk_backgroundColor
    ...
    fileprivate func getCurrenStyleImage() ->UIImage{
        ...
        if isJonit {
            return self.jointView.collectionView.scrollViewShot()!
        }else {
    ...
    fileprivate func createJointView() {
        ...
        jointView.imageExchange = {[weak self] from,to in
            guard let `self` = self else {return}
            let currentImage = self.images[from]
            self.images.remove(at: from)
            self.images.insert(currentImage, at: to)
            self.jiasawView.updateImages(images:self.images)
        }
    ...
    fileprivate func createStyleView() {
        ...
        styleView.backgroundColor = .zsk_groupedBackgroundColor
