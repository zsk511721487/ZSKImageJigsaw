...
class ZSKImageStyleView: UIView {
    ...
    lazy var layoutButton: UIButton = {
        ...
        btn.setTitleColor(.zsk_titleColr, for: .normal)
        btn.showsTouchWhenHighlighted = false
    ...
    lazy var collectionView: UICollectionView = {
        ...
        let cv = UICollectionView(frame: CGRect(x: 15, y: self.bounds.height - 55 - zskjigsaw_bottom_safe_height, width: zskjigsaw_screen_width - 30, height: 45), collectionViewLayout: layout)
        cv.delegate = self
        cv.dataSource = self
        cv.backgroundColor = .zsk_groupedBackgroundColor
    ...
    lazy var jointImageView:UIImageView = {
        let imageView = UIImageView(frame: CGRect(x: 15, y: 0, width: 40, height: 50))
        imageView.contentMode = .scaleAspectFill
        imageView.clipsToBounds = true
        imageView.backgroundColor = .zsk_groupedBackgroundColor
        return imageView
    }()
